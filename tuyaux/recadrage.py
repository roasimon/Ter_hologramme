...
im_path = "dataset\images\origins\passport\psp01_01_01" # chemin vers le dossier d'image d'un passeport
corner_path = "dataset\markup\origins\passport\psp01_01_01" # chemin vers le dossier des corners du même passeport

lst_im = os.listdir(im_path)[:-1] # liste des images
lst_corners = os.listdir(corner_path) # liste des corners
...
def changePerspecAllImages(im_dir: str, corners_dir: str):
    lst_im = os.listdir(im_dir)[:-1] # liste des images
    lst_corners = os.listdir(corners_dir) # liste des corners

    for i in range(len(lst_im)):
        img = cv2.imread(os.path.join(im_path, lst_im[i])) # on récupère l'image
        with open(os.path.join(corner_path, lst_corners[i])) as f: # on récupère le fichier json qui contient les corners
            data = json.load(f)["document"]["templates"]["uto.passport.type1:main"]["template_quad"] # on récupère les corners
        ...
        cv2.imwrite(f"warped_{i}", warped)
